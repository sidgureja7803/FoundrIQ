...
const IdeaSubmissionPage: React.FC = () => {
  ...
  const handleAnalyzeClick = async () => {
    ...
    setLoadingText('Analyzing your idea and generating questions...');
    setError(null);
    ...
    try {
      ...
      if (error.message?.includes('API key') || 
        ...
        setError('⚠️ AI Service Configuration Error: ' + error.message + ' Please contact the administrator to configure IBM Granite or Perplexity API keys.');
      } else {
        setError(error.message || "Failed to analyze your idea. Please try again or contact support if the issue persists.");
      }
      setStep('input');
    }
  };
  ...
  const handleSubmitAnswers = async () => {
    ...
    try {
      ...
      if (error.message?.includes('API key') || 
          error.message?.includes('unavailable') ||
          error.message?.includes('authentication')) {
        setError('⚠️ AI Service Error: ' + error.message);
      } else {
